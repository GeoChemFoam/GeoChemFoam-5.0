...
TotalTime=0.0025
WriteTimestep=0.0001
initTimestep=0.00000001
maxTimestep=0.0000001
maxCourant=0.2
...
then
    ...
    echo "Run interTransportFoam in parallel on $NP processors"
    mpiexec -np $NP interTransportFoam -parallel > interTransportFoamRun.out
else
    echo "Run interTransportFoam"
    interTransportFoam > interTransportFoamRun.out
fi
