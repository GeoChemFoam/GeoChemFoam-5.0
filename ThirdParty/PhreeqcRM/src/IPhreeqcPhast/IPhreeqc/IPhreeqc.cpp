...
const char* IPhreeqc::GetSurfaceComponent(int n)
{
	static const char empty[] = "";
	this->ListSurfaceComponents();
	if (n < 0 || n >= (int)this->SurfComponents.size())
	{
		return empty;
	}
	std::list< std::string >::iterator it = this->SurfComponents.begin();
	for (int i = 0; i < n; ++i)
	{
		++it;
	}
...
size_t IPhreeqc::GetSurfaceComponentCount(void)
{
	this->ListSurfaceComponents();
	return this->SurfComponents.size();
}
...
std::list< std::string > IPhreeqc::ListSurfaceComponents(void)
{
	if (this->UpdateSurfaceComponents)
	{
		this->SurfComponents.clear();
		this->PhreeqcPtr->list_surface_components(this->SurfComponents);
		this->UpdateSurfaceComponents = false;
	}
	return this->SurfComponents;
}
...
void IPhreeqc::UnLoadDatabase(void)
{
	...
	this->UpdateSurfaceComponents = true;
	this->Components.clear();
